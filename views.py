...
class UsuarioViewSets(viewsets.ModelViewSet):
    ...
    # Em vez de usuario.delete(), fazemos a alteração manual:
        usuario.ativo = False
        usuario.save()
    
        return Response({
                    'mensagem': f'Usuário {nome} desativado com sucesso'
            }  , status=status.HTTP_200_OK)
    ...
    def endereco(self, request):
        '''
        GET: Retorna o endereço do usuário autenticado
        PATCH: Atualiza o endereço do usuário autenticado
        '''
        # Pegar o id do usuario dentro do token
        user_id = request.auth.payload.get('user_id')
        ...
        try:
            ...
            # Se for GET, retorna o endereço atual
            if request.method == 'GET':
                serializer = CepSerializer(usuario)
                return Response(serializer.data,
                                status=status.HTTP_200_OK)

            # Se for PATCH, atualiza o endereço
            elif request.method == 'PATCH':
                serializer = CepSerializer(
                    usuario,
                    data=request.data,
    ...
    @action(detail=False, methods=['get'],
            url_path='listar-com-filtros',
            permission_classes=[AllowAny])
    def listar_com_filtros(self, request):
        '''
        GET: Lista usuários com filtros opcionais
        Parâmetros de query:
        - email: filtra por email (busca parcial)
        - nome: filtra por nome (busca parcial)
        - ordem: ordena resultados (padrão: -criado)

        Exemplo: /usuarios/listar-com-filtros/?nome=joao&ordem=nome
        '''
        # 1º passo - pegar todos os usuários
        usuarios = Usuario.objects.all()

        # 2º passo - filtrar por email (se fornecido)
        ...
        # 3º passo - filtrar por nome (se fornecido)
        ...
        campos_validos = ['nome', '-nome', 'email', '-email',
                          'criado', '-criado', 'atualizado', '-atualizado']
        if ordem in campos_validos:
            usuarios = usuarios.order_by(ordem)
        else:
            usuarios = usuarios.order_by('-criado')

        # 5º passo - serializar os dados
