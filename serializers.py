...
class CadastroSerializer(
    ...
    class Meta:
        ...
        fields = ['id', 'nome', 'cpf', 'cep', 'logradouro', 'numero', 'complemento',
            'bairro', 'cidade', 'estado', 'email', 'senha', 'criado', 'atualizado',
            'senha_confirmacao']
    ...
    def validate_senha(self, value):
        ...
        if value.isupper():
            raise serializers.ValidationError(
                'Senha nao pode ser apenas letras maiúsculas'
            )
        if len(value) < 8:
            raise serializers.ValidationError(
    ...
    def validate_cpf(self, value):
        """
        Valida CPF: aceita com ou sem pontos e traço.
        Guarda apenas números no banco.
        """
        # Remove tudo que não é número
        numeros = re.sub(r'\D', '', value)  # \D = tudo que NÃO é dígito

        # Verifica se restaram 11 números
        if len(numeros) != 11:
            raise serializers.ValidationError("CPF inválido. Deve ter 11 números.")

        return numeros  # retorna apenas os números, que serão salvos no banco
...
class UsuarioSerializer(serializers.ModelSerializer):
    '''
    Serializer para retornar dados completos do usuário (perfil)
    '''
    class Meta:
        model = Usuario
        fields = ['id', 'nome', 'email', 'cpf', 'cep', 'logradouro', 'numero',
                  'complemento', 'bairro', 'cidade', 'estado', 'criado']
        read_only_fields = ['id', 'criado']
...
class CepSerializer(serializers.ModelSerializer):
    '''
    Serializer para atualizar/visualizar endereço do usuário
    '''
    class Meta:
        model = Usuario
    ...
    def validate_cep(self, value):
        # Validar CEP (apenas números, 8 dígitos)
        cep_limpo = value.strip()
